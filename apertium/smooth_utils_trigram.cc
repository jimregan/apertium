...
double 
SmoothUtilsTrigram::lambda(double count) {
  double sqrtval=sqrt(count);
  return sqrtval/(1+sqrtval);
}

map<int, double> 
SmoothUtilsTrigram::prob_amb_class_from_tag(TaggerDataTrigram& tagger_data, 
				     int i, 
...
SmoothUtils::prob_amb_class_from_tag2(TaggerData& tagger_data,
				      int i,
				      map <int , <map <int, double> > &prob_ambclass2) {
  double sum_ambclass=0.0;
  map<int, map<int, double> > prob_ambclass_tag2;

  for(int k=0; k<tagger_data.getM(); k++) {
    if (tagger_data.getOutput()[k].find(i)!=tagger_data.getOutput()[k].end()) {
      sum_ambclass+=prob_ambclass2[j][k];
    }
  }

  for(int j=0; j<tagger_data.getM(); j++) {
    for(int k=0; k<tagger_data.getM(); k++) {
      if (tagger_data.getOutput()[k].find(i)!=tagger_data.getOutput()[k].end()) {
        prob_ambclass_tag2[j][k]=prob_ambclass2[j][k].....;
      }
    }
  }
...
*/
void
SmoothUtilsTrigram::calculate_smoothed_parameters(TaggerDataTrigram& tagger_data,
  ...
  map<int,  map<int, double> > prob_tag2;
  ...
  for (int i=0; i<tagger_data.getN(); i++) {
	...
	frec_ij=0.0;
      ...
      prob_tag2[i][j]=(lambda_value*frec_ij) +	((1.0-lambda_value)*prob_tag[j]);
  ...
  for (int i=0; i<tagger_data.getN(); i++) {
    for (int j=0; j<tagger_data.getN(); j++) {
      for (int k=0; k<tagger_data.getN(); k++) {
	double lambda2 = lambda(tags_pairs[i][j]);
	double frec_ijk;
	if (tags_pairs[i][j] > 0)
	   frec_ijk = tags_triple[i][j][k]/tags_pairs[i][j];
	else frec_ijk = 0.0;
	tagger_data.getA()[i][j][k] = (lambda2*frec_ijk) + (1.0-lambda2)*prob_tag2[j][k];
      }
    }
  }
  ...
  for(int j=0; j<tagger_data.getN(); j++) {
	...
	prob_ambclass2[j][k]=(lambda_value*frec_jk) + ((1.0-lambda_value)*prob_ambclass_tag[k]);
  ...
  for(int i=0; i<tagger_data.getN(); i++) {
     //prob_ambclass_tag2 = prob_amb_class_from_tag2(tagger_data, i, prob_ambclass2);
     for(int j=0; j<tagger_data.getN(); j++) {
  	for(int k=0; k<tagger_data.getM(); k++) {
      	   if(tagger_data.getOutput()[k].find(j)!=tagger_data.getOutput()[k].end()) {
		double lambda2 = lambda(tags_pairs_for_emis2[i][j]);
		double frec_ijk;
		if(tags_pairs_for_emis2[i][j]>0)
		  frec_ijk = emis2[i][j][k]/tags_pairs_for_emis2[i][j];
		else
		  frec_ijk=0.0;
		tagger_data.getB()[i][j][k] = (lambda2*frec_ijk) + (1.0-lambda2)*prob_ambclass2[j][k];
	   }
        }
     }
  }
...
void 
SmoothUtilsTrigram::save_counts(TaggerDataTrigram& td, const string& filename,
                                           map<int, double> &tags_count,
                                           map<int, map<int, double> > &tags_pairs,
                                           map<int, map<int, map<int, double> > > &tags_triple,
                                           map<int, double> &ambclass_count,
  ...
  for (int i=0; i<td.getN(); i++) {
    for(int j=0; j<td.getN(); j++) {
      EndianDoubleUtil::write(fcounts, tags_pairs[i][j]);
    }
  }
  ...
  for (int i=0; i<td.getN(); i++) {
    for(int j=0; j<td.getN(); j++) {
      for(int k=0; k<td.getN();k++){ 
        EndianDoubleUtil::write(fcounts, tags_triple[i][j][k]);
      }
    }
  }
  ...
  for(int i=0; i<td.getN(); i++) {
    for(int j=0; j<td.getN(); j++) {
      for(int k=0; k<td.getM(); k++) {
        if (td.getOutput()[k].find(i)!=td.getOutput()[k].end()) {
          Compression::multibyte_write(i, fcounts);
          Compression::multibyte_write(j, fcounts);
          Compression::multibyte_write(k, fcounts);
          EndianDoubleUtil::write(fcounts, emis2[i][j][k]);
        }
      }
    }
  }
  ...
  for (int i=0; i<td.getN(); i++) {
    for (int j=0; j<td.getN(); j++) {
      EndianDoubleUtil::write(fcounts, tags_pairs_for_emis2[i][j]);
    }
  }
