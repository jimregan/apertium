...
void
SWPoST::set_eos(TTag t) { 
  eos = t; 
} 

void
SWPoST::set_debug(bool d) { 
  debug = d; 
} 

void
SWPoST::set_show_sf(bool sf) { 
  show_sf = sf; 
}
 
void
SWPoST::setNullFlush(bool nf) {
  null_flush = nf;
}

void
SWPoST::init_probabilities(FILE *ftxt) {
  int N = td->getN();
  int M = td->getM();
...
void
SWPoST::read_dictionary(FILE *fdic) {
  int i, k, nw = 0;
...
SWPoST::train(FILE *ftxt) {
  ...
  vector<vector<vector<double> > > para_matrix_new(M, vector<vector<double> >(M, vector<double>(N, 0)));
...
void 
SWPoST::tagger(FILE *in, FILE *out, bool show_all_good_first) {
  int s_left, s_right;
