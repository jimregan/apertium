...
void
TaggerData::setSWPoSTProbabilities(int const myN, int const myM, double ***myC) {
   this->destroy();
   N = myN;
   M = myM;
   if(N != 0 && M != 0) {
...
void
TaggerData::setLSWPoSTProbabilities(int const myN, double ***myD) {
   this->destroy();
   N = myN;
   if(N != 0) {
     d = new double ** [N];
...
void
TaggerData::readLSWPoST(FILE *in)
{
  destroy();
...
void
TaggerData::writeLSWPoST(FILE *out)
{
