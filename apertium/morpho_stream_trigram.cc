...
#include "morpho_stream_trigram.h"
...
MorphoStreamTrigram::MorphoStreamTrigram(FILE *ftxt, bool d, TaggerDataTrigram *t)
{
  foundEOF = false;
  debug=d;
  td = t;
...
MorphoStreamTrigram::~MorphoStreamTrigram() 
{
  delete me;
}

TaggerWord *
MorphoStreamTrigram::get_next_word()
{
  if(vwords.size() != 0)
  {
...
void
MorphoStreamTrigram::lrlmClassify(wstring const &str, int &ivwords)
{
  int floor = 0;
  int last_type = -1;
  int last_pos = 0;
...
void
MorphoStreamTrigram::readRestOfWord(int &ivwords)
{
